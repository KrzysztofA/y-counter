...
class AlarmCreator:
    def __init__(self, app, parent=None):
        ...
        self.sound_files = ["neraton.mp3"]
        self.sound_file = StringVar(self.window, value=self.sound_files[0])
        self.combo_box = Combobox(self.window, values=self.sound_files, textvariable=self.sound_file)
        self.combo_box.grid(column=1, row=6, columnspan=1)

        def add_and_close():
            app.add_alarm(Alarm(self.sound_file.get(), parent, self.mode_var.get(), self.count_var.get(), self.time_var.get() * 60))
            self.window.destroy()
